...
def solver(
    ...
    Q = jnp.tile(jnp.eye(state_space.shape[0]) * noise_var, (A.shape[0], 1, 1))
...
def rollout(
    ...
    def episode(carry, time):
        key, state = carry
        policy_key, noise_key, key = jr.split(key, 3)

        action = action_space.clip(policy(state, time, policy_key))        
        cost = transient_cost(state, action, time, goal_state)        
        rv = noise_var * jax.random.normal(noise_key, shape=state_space.shape)
